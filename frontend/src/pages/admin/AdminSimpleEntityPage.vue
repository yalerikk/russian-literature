  ...
  <div class="admin-page">
    <div class="admin-content">
      ...
      <div class="page-header">
        ...
        <button class="add-button" @click="openCreateModal">Добавить {{ entityName.toLowerCase() }}</button>
      </div>
      ...
      >
        ...
        <template v-if="hasType" #cell-type="{ value }">{{ typeLabels[value] || value }}</template>
...
const columns = [
  ...
  ...(props.hasType ? [{ key: 'type', label: 'Тип' }] : []),
  { key: 'actions', label: 'Действия', className: 'actions-cell' }
]

const typeLabels = { GRADE: 'Класс', LEVEL: 'Уровень', CATEGORY: 'Категория', READING_TYPE: 'Тип чтения' }

const { items, loading, currentPage, totalPages, loadItems, onPageChange, deleteItem } = useAdminCrud(props.fetchUrl, props.apiBaseUrl, 10)
const modal = ref(null)
const confirmModal = ref(null)

function openCreateModal() { modal.value.open() }
