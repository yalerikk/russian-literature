  ...
  <div class="admin-page">
    <div class="admin-content">
      <div class="breadcrumb-nav">
        ...
        <span class="breadcrumb-item current">Авторы</span>
      </div>
      ...
      <div class="table-wrapper" v-if="!loading">
        <div class="table-header">
          ...
          <div class="header-cell">Даты жизни</div>
          <div class="header-cell actions-cell">Действия</div>
        </div>

        <div class="table-body">
          <div v-if="authors.length === 0" class="empty-row">
            <div class="empty-icon">👤</div>
            <h3>Авторов пока нет</h3>
            <p>Добавьте первого автора</p>
          </div>

          <div v-for="author in authors" :key="author.id" class="table-row">
            <div class="row-cell">{{ formatFullName(author) }}</div>
            <div class="row-cell">{{ formatBirthDeath(author) }}</div>
            <div class="actions-cell">
              <button class="action-btn edit" @click="openEditModal(author)">
    ...
    <AuthorModal ref="authorModal" @saved="loadAuthors" />
...
async function loadAuthors() {
  loading.value = true
  try {
    const res = await apiClient.get(`/authors/admin/list?page=${currentPage.value}&size=${pageSize}`)
...
function openCreateModal() {
  if (authorModal.value) {
    authorModal.value.open()
  } else {
    console.error('authorModal ref is null')
  }
}

function openEditModal(author) {
  authorModal.value.open(author)
}
...
function formatBirthDeath(author) {
