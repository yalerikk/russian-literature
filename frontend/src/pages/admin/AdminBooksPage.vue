  ...
  <div class="admin-page">
    <div class="admin-content">
      <div class="breadcrumb-nav">
        <router-link to="/profile" class="breadcrumb-item">Профиль</router-link>
        <span class="separator">/</span>
        <span class="breadcrumb-item current">Книги</span>
      </div>

      <div class="page-header">
        <h1 class="page-title">Книги</h1>
        <button class="add-button" @click="openCreateModal">Добавить книгу</button>
      </div>
      ...
      <BookModal ref="bookModal" @saved="loadItems" />
...
const columns = [
  ...
  { key: 'publicationYear', label: 'Год' },
  { key: 'actions', label: 'Действия', className: 'actions-cell' }
]

const { items, loading, currentPage, totalPages, loadItems, onPageChange, deleteItem } = useAdminCrud('/books/admin/list', '/books', 10)
const bookModal = ref(null)
const confirmModal = ref(null)

function openCreateModal() { bookModal.value?.open() }
function openEditModal(book) { bookModal.value?.open(book) }
