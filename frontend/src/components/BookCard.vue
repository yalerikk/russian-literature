    ...
    <div class="book-info">
      ...
      <div class="book-stats">
        ...
        >
          ...
          <svg 
            v-else width="18" height="18" viewBox="0 0 20 20" fill="none" 
          >
            <!-- SVG пустого сердца -->
...
export default {
  ...
  setup(props) {
    ...
    const isFavorite = computed(() => favorites.isFavorite(props.book.id));
    const handleToggleFavorite = async (e) => {
      e.stopPropagation();
      if (!authService.isAuthenticated.value) {
        if (confirm('Войдите, чтобы продолжить')) {
          eventBus.dispatchEvent(new CustomEvent('show-auth-modal'));
        }
        return;
      }
      try {
        if (isFavorite.value) {
          await favorites.removeFavorite(props.book.id);
          if (window.location.pathname === '/profile/favorites') {
            window.location.reload();
          }
        } else {
          await favorites.addFavorite(props.book.id);
        }
      } catch (err) {
    ...
    const coverImageUrl = computed(() => props.book.coverUrl?.trim() ? props.book.coverUrl : '/images/cover.png');
    const handleImageError = (e) => { e.target.src = '/images/cover.png'; };

    return { isFavorite, handleToggleFavorite, coverImageUrl, handleImageError };
  },
};
</script>
