<template>
  <div class="auth-form">
    ...
    <div class="button-group">
      <button class="auth-button" @click="handleSubmit" :disabled="loading">
        {{ loading ? 'Вход...' : 'Войти' }}
      </button>
    </div>
    ...
    <div class="footer-links">
      ...
      <button class="link-button" @click="$emit('switch-to-register')">Зарегистрируйтесь!</button>
    </div>
  </div>
</template>
...
async function handleSubmit() {
  ...
  passwordError.value = ''
  if (!login.value.trim()) {
    loginError.value = 'Введите логин или email'
    return
  }
  if (!password.value) {
...
/* Скрываем нативную иконку менеджера паролей на macOS */
